...
class BookAPITest(BaseBookTest):
  ...
  def test_create(self):
    for i in range(5):
      post_data = {
        ...
        'author': str(random.randomint(10000, 99999)),
  ...
  def test_list(self):
    ...
    for i in range(len(book_qs)):
      ...
      self.verify_book(book, book, dict)
  ...
  def test_delete(self):
    book_list = self.book_finished_list + self.book_unfinshed_list
    for book in book_list:
      url = reverse('book-detail', kwargs={'pk': book.id})
